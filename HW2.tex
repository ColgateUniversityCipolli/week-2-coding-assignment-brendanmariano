...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hldef{right.vals} \hlkwb{=} \hlkwd{integrand}\hldef{(right.points)}
\hldef{left.vals} \hlkwb{=} \hlkwd{integrand}\hldef{(left.points)}
\hldef{(trap.area} \hlkwb{=} \hlkwd{sum}\hldef{(delta.x}\hlopt{*}\hldef{(right.vals} \hlopt{+} \hldef{left.vals)}\hlopt{/}\hlnum{2}\hldef{))}
\end{alltt}
\begin{verbatim}
## [1] 8.6664
\end{verbatim}
...
\hldef{riemann.sums} \hlkwb{<-} \hlkwa{function}\hldef{(}\hlkwc{fnct}\hldef{,}                        \hlcom{# function to integrate}
  ...
  \hlkwa{if}\hldef{(method} \hlopt{==} \hlsng{"Left"}\hldef{)\{}
    \hlcom{#Pre-established points that are used}
    \hlcom{#Used sum function in order to sum every computed rectangle}
    \hldef{area} \hlkwb{=} \hlnum{0}
    \hldef{increments.x} \hlkwb{=} \hldef{(b}\hlopt{-}\hldef{a)}\hlopt{/}\hldef{n.rect}
    \hldef{left.points} \hlkwb{=} \hldef{a} \hlopt{+} \hlnum{0}\hlopt{:}\hlnum{99}\hlopt{*}\hldef{(increments.x)}
      \hldef{area} \hlkwb{=} \hlkwd{sum}\hldef{(increments.x}\hlopt{*}\hldef{(}\hlkwd{fnct}\hldef{(left.points)))}

  \hldef{\}}\hlkwa{else if}\hldef{(method} \hlopt{==} \hlsng{"Right"}\hldef{)\{}
    \hldef{increments.x} \hlkwb{=} \hldef{(b}\hlopt{-}\hldef{a)}\hlopt{/}\hldef{n.rect}
    \hldef{right.points} \hlkwb{=} \hldef{a} \hlopt{+} \hlnum{1}\hlopt{:}\hlnum{100}\hlopt{*}\hldef{(increments.x)}
    \hldef{area} \hlkwb{=} \hlkwd{sum}\hldef{(increments.x}\hlopt{*}\hldef{(}\hlkwd{fnct}\hldef{(right.points)))}

  \hldef{\}}\hlkwa{else if}\hldef{(method} \hlopt{==} \hlsng{"Midpoint"}\hldef{)}
    \hldef{\{}
    \hlcom{#Used same idea as above but had to calculate both right and left values for each interval to find the midpoint Riemann Sum}
    \hldef{increments.x} \hlkwb{=} \hldef{(b}\hlopt{-}\hldef{a)}\hlopt{/}\hldef{n.rect}
    \hldef{right.points} \hlkwb{=} \hldef{a} \hlopt{+} \hlnum{1}\hlopt{:}\hlnum{100}\hlopt{*}\hldef{(increments.x)}
    \hldef{left.points} \hlkwb{=} \hldef{a} \hlopt{+} \hlnum{0}\hlopt{:}\hlnum{99}\hlopt{*}\hldef{(increments.x)}
    \hldef{mid.points} \hlkwb{=} \hldef{a} \hlopt{+} \hldef{(left.points} \hlopt{+} \hldef{right.points)}\hlopt{/}\hlnum{2}
    \hldef{area} \hlkwb{=} \hlkwd{sum}\hldef{(increments.x}\hlopt{*}\hldef{(}\hlkwd{fnct}\hldef{(mid.points)))}
  \hldef{\}}\hlkwa{else if}\hldef{(method} \hlopt{==} \hlsng{"Trapezoidial"}\hldef{)\{}
     \hlcom{#Averages the two values, multiplies by the width and that adds to area }
    \hlcom{#Used sum function to add all of the trapezoids}
      \hldef{increments.x} \hlkwb{=} \hldef{(b}\hlopt{-}\hldef{a)}\hlopt{/}\hldef{n.rect}
      \hldef{right.points} \hlkwb{=} \hldef{a} \hlopt{+} \hlnum{1}\hlopt{:}\hlnum{100}\hlopt{*}\hldef{(increments.x)}
    \hldef{left.points} \hlkwb{=} \hldef{a} \hlopt{+} \hlnum{0}\hlopt{:}\hlnum{99}\hlopt{*}\hldef{(increments.x)}
      \hldef{left.vals} \hlkwb{=} \hldef{(}\hlkwd{fnct}\hldef{(left.points))}
      \hldef{right.vals} \hlkwb{=} \hldef{(}\hlkwd{fnct}\hldef{(right.points))}

      \hldef{area} \hlkwb{=} \hlkwd{sum}\hldef{((increments.x}\hlopt{*}\hldef{(left.vals} \hlopt{+} \hldef{right.vals))}\hlopt{/}\hlnum{2}\hldef{)}
