...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hldef{(trap.area} \hlkwb{=} \hlnum{1}\hlopt{/}\hlnum{2}\hlopt{*}\hldef{(left.area} \hlopt{+} \hldef{right.area))}
\end{alltt}
\begin{verbatim}
## [1] 8.6664
\end{verbatim}
...
\hldef{riemann.sums} \hlkwb{<-} \hlkwa{function}\hldef{(}\hlkwc{fnct}\hldef{,}                        \hlcom{# function to integrate}
  ...
  \hlkwa{if}\hldef{(method} \hlopt{==} \hlsng{"Left"}\hldef{)\{}
    \hldef{increments.x} \hlkwb{=} \hldef{(upper}\hlopt{-}\hldef{lower)}\hlopt{/}\hldef{n.rect}
    \hlkwa{for}\hldef{(i} \hlkwa{in} \hldef{n.rect)}
      \hlkwd{print}\hldef{(i)}
      \hldef{area} \hlkwb{=} \hldef{area} \hlopt{+} \hlkwd{increments.x}\hldef{(}\hlkwd{func}\hldef{(lower} \hlopt{+} \hldef{i}\hlopt{/}\hldef{n.rect))}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in riemann.sums(fnct = integrand, a = 0, b = 2, n.rect = 100): object 'area' not found}}\begin{alltt}
\hlcom{######################################}
